...
win.addEventListener("load",function()
{
	...
	xhr.onerror = function(e)
		...
		//	data.push({leftImage:'/image/cha_pic2_1x.png',title:venues[i].name, hasChild:true});
			data.push({title:venues[i].name, hasChild:true});
			//data.push({title:venues[i].name});


			annotations.push(annotation);

			anno_search.push(annotation);
...
var search_btn = Titanium.UI.createButton({
	//backgroundImage:'/image/search box _ 2x.png',
	top:180,
	left:350,
	title:'Search',
	height:60,
...
search_btn.addEventListener('click',function(e)
{
    ...
    for (i=annotations.length-1;i>=0;i--) {
        mapview.removeAnnotation(annotations[i].title);
      //  alert(annotations[i].title);
    ...
    data=[];
       for (var i=0;i<10;i++)
				{
					var str=venues[i].name;
					//venues[i].name=='Genpact'
					var searchtext=search.value;
				if(str.indexOf(searchtext.trim()) >= 0)
				{
					//alert(venues[i].name);
				var annotation = Titanium.Map.createAnnotation({
			    latitude:venues[i].location.lat, longitude:venues[i].location.lng,
...
//win.add(mapview1);
//mapview1.setMapType(Titanium.Map.SATELLITE_TYPE);
